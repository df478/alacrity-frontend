...
> {
                            ...
                            onOk() {
                                if (isUsingHttp) {
                                    window.location.replace(
                                        `https://${self.state.apiData.alacranSubDomain}.${self.state.apiData.rootDomain}`
                                    )
                                }
                            },
                            onCancel() {
                            ...
                            <div>
                                <p>
                                    {localize(
                                        ...
                                        "AlaCrity uses Let's Encrypt to provide free SSL Certificates (HTTPS)."
                                    )}
                                    {localize(
        ...
        this.apiManager
            ...
            .then(function (data: any) {
                Modal.success({
                    ...
                    onOk() {
                        window.location.replace(
                            `http://${self.state.apiData.alacranSubDomain}.${rootDomain}`
                        )
                    },
                })
            })
    ...
    createSetupPanelIfNoForceSsl() {
        ...
        const replacements = [
            ...
            <i> alacran.myawesomecompany.com </i>,
            <i> foo.bar.myawesomecompany.com </i>,
            <Fragment>
                <br />
                <b> Type:</b> <u>A</u>, <b>Name (or host):</b>{' '}
                <u> *.alacrity-root</u>, <b> IP (or Points to):</b>{' '}
                <u> 110.120.130.140 </u>
                <br />
            </Fragment>,
        ]
        const translatedBody = Utils.formatText(
            localize(
                ...
                'The very first thing that AlaCrity needs is a root domain. For example, if you own %s1, you can use %s2 or %s3 as your root domain. First, you need to make sure that the ip address for all subdomains of the root domain resolve to the AlaCrity ip address. To do this, go to the DNS settings in your domain provider website, and set a wild card A entry. For example: %s4 where this IP is the IP address of your AlaCrity machine (server).'
            ),
            ['%s1', '%s2', '%s3', '%s4'],
            replacements
        )
                ...
                <Col xs={{ span: 23 }} lg={{ span: 16 }}>
                    <Card
                        title={localize(
                            ...
                            'AlaCrity Root Domain Configurations'
                        )}
                    >
                        <div>
                            <p>{translatedBody}</p>
                        ...
                        <div>
                            ...
                            <p>
                                ...
                                {localize(
                                    ...
                                    'You have installed AlaCrity successfully!'
                                )}{' '}
                                <b>
                                    {localize(
                                        'dashboard.https_setup_needed',
                                        'But you still need to assign a domain and finish the HTTPS setup to fully set up AlaCrity!'
                                    )}
                                </b>
                                {localize(
                                    'dashboard.setup_options',
                                    'You can set up your AlaCrity instance in two ways:'
                                )}
                            </p>
                            ...
                            <ul>
                                <li>
                                    ...
                                    <code>
                                        {localize(
                                            'dashboard.alacrity_serversetup',
