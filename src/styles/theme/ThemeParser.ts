...
import AlaCrityTheme, { AlaCrityExtraTheme } from './AlaCrityTheme'
...
const ThemeParser = {
    parseTheme(
        currTheme: AlaCrityTheme | undefined,
        isDarkMode: boolean,
    ...
    parseExtra(currTheme: AlaCrityTheme | undefined) {
        if (!currTheme) return undefined

        const context = {}
        return (parseObject(currTheme, context) as AlaCrityTheme)?.extra as
            | AlaCrityExtraTheme
            | undefined
    },
}
